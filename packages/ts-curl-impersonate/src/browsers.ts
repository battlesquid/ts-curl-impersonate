...
export const BINARY_PATH = path.join(
	path.dirname(
		require.resolve(`ts-curl-impersonate-bin-${process.platform}-${process.arch}/package.json`),
	),
	"bin",
);
...
export const getCompatibleBrowsers = () => {
	...
	if (matchingArchs.length === 0) {
		...
		throw new Error(
			`Unable to find browser binary that matches system architecture (system: ${process.arch}, available: ${availableArchs})`,
		);
	}
...
export const resolveBrowser = (browser: BrowserType) => {
	...
	if (foundBrowser === undefined) {
		...
		throw new Error(
			`Unable to find browser with name '${browser}' (available: ${availableNames})`,
		);
	}
	return foundBrowser;
};
